    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --presets|-p)
                ...
                echo ""
                echo "  ornith     Ornith 1.0 397B MoE flagship (Deep Reinforce, Qwen3.5)"
                echo "             vLLM: main (qwen3_5_moe + MTP), transformers>=5.8.1, NGC 26.05"
                echo "             FP8 ~400GB, native MTP (PP-gated), 256K ctx. 2 nodes × 4"
                echo "             (TP=4 + PP=2). qwen3_xml/qwen3 parsers. Work-tier weights."
                echo ""
                echo "  ornith_gh200  Ornith 1.0 35B MoE FP8 (~3B active) on a SINGLE GH200 card"
                echo "             TP=1, MTP on, 256K ctx — max single-user throughput. Shares"
                echo "             the ornith container. Aliases: ornith-35b, ornith-1gpu."
                echo ""
