...
- **Wall-clock is dominated by eager multi-node decode driving a *single* request stream** — the benchmark is sequential, so the GPUs batch nothing (glm52 per-instance ranged 39 s … 28 min). A parallel sharded mode (K runner shards → one `--no-serialize` proxy → router → vLLM batches K streams) would cut this ~K×; not yet built. mini-swe-agent on the multi-node models was deferred for the same reason — the bare loop's `anthropic_proxy` gives the serialization + keepalive that wedge-prone multi-node PP models want and the litellm→router path lacks.
